...
test('getDefaultReceiptSystemPrompt returns non-empty prompt matching categories', () => {
  ...
  for (const category of RECEIPT_CATEGORIES) {
    assert.ok(prompt.includes(category), `System prompt must mention category '${category}'`);
  }
...
test('parseReceipt and isReceiptCached enforce 50KB input limit', async () => {
  ...
  assert.strictEqual(isReceiptCached(oversizedText, { model: 'test-model' }), false);
  ...
  assert.strictEqual(isReceiptCached(cyrillicOversized, { model: 'test-model' }), false);
  await assert.rejects(
    () =>
...
test('isReceiptCached and parseReceipt reuse raw JSONs in cache for example receipts', async () => {
  ...
  assert.strictEqual(isReceiptCached(EXAMPLE_RECEIPT_TEXT_1, { model, noCache: true }), false);
  ...
  try {
    ...
    assert.deepStrictEqual(res.evm_wallets, ['0x9793Dd93D46F153a2A879165cd163A01b54d8A00']);
