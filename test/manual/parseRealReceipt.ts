...
import { isReceiptCached, parseReceipt } from '../../src/oracle/receiptParser.js';
...
async function runTestAi(): Promise<void> {
  ...
  const receiptSource = args.find(
    (arg, i) => !arg.startsWith('-') && args[i - 1] !== '-o' && args[i - 1] !== '--output',
  );
  ...
  const outputFlagIdx = args.findIndex((arg) => arg === '-o' || arg === '--output');
  const outputFile =
  ...
  const isUrl = receiptSource.startsWith('http://') || receiptSource.startsWith('https://');
  ...
  if (isUrl) {
    ...
    console.log(`Loaded receipt file (${receiptText.length} characters) from ${resolvedPath}.`);
  }
